...
inline void handleAbortRequest() {
  ...
  // If the flag is not found, we have removed in HipAbort to denote abort is not
  // called in the module. This is used for avoiding kernel launches to read the
  // value to minimize overheads when abort is not used.
  hipError_t Err =
    hipMemcpyFromSymbol(&DeviceRequestedAbort, HIP_SYMBOL(__chipspv_abort_called),
			sizeof(int32_t));
  if (Err != hipErrorInvalidSymbol && DeviceRequestedAbort) {
    ...
    if (getenv("CHIP_HOST_IGNORES_DEVICE_ABORT") == nullptr) {
      ...
      hipMemcpyToSymbol(HIP_SYMBOL(__chipspv_abort_called),
			&DeviceRequestedAbort, sizeof(int32_t),
			0, hipMemcpyHostToDevice);
      fprintf(stderr, "[ABORT IGNORED]\n");
    }
  }
}
