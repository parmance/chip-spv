...
static ze_image_type_t getImageType(unsigned HipTextureID) {
  ...
  assert(false && "Unknown or unsupported HIP texture type.");
  return ZE_IMAGE_TYPE_2D;
}
...
CHIPTexture *CHIPDeviceLevel0::createTexture(
  ...
  if (PResDesc->resType == hipResourceTypeArray) {
    ...
    assert(Array->data && "Invalid hipArray.");
    assert(!Array->isDrv && "Not supported/implemented yet.");
  ...
  assert(false && "Unsupported/unimplemented texture resource type.");
  return nullptr;
}
...
void CHIPExecItem::setupAllArgs() {
  ...
  if (ArgsPointer_) {
    ...
    for (size_t i = 0; i < OffsetSizes_.size(); ++i) {
      ...
      assert(ArgTypeInfo.Type != OCLType::Image && "UNIMPLEMENTED: texture "
                                                   "object arguments for old "
                                                   "HIP kernel launch API.");
