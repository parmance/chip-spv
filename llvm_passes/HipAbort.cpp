...
// device busy and reading the global variable might not work (since it currently
// requires kernel execution).
//
...
PreservedAnalyses HipAbortPass::run(Module &Mod, ModuleAnalysisManager &AM) {
  ...
  auto *Int32ZeroConst = ConstantInt::get(Int32Ty, 0);
  ...
  for (auto Call : CallsToHandle) {
    ...
    LoadInst* FlagLoad =
        B.CreateLoad(
            AbortFlag->getValueType(), AbortFlag, true, "abort_flag");
    Value* FlagCmp =
        B.CreateICmpNE(
  ...
  for (auto Call : AbortCallsToHandle) {
    ...
    IRBuilder<> B(Call);
    StoreInst* FlagStore = B.CreateStore(Int32OneConst, AbortFlag, true);
